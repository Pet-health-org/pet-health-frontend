...
import { findByMascota as getVacunasByMascota } from "../../../services/vacuna.service";
...
export function PetProfilePage() {
    ...
    const fetchFullProfile = async () => {
      try {
        ...
        const [petRes, historyRes, vacunasRes] = await Promise.all([
          ...
          getHistoriasClinicasByMascota(id as string).catch(() => ({
            data: [],
          })),
        ...
        const ownerId =
          petData.propietarioId ||
          petData.duenoId ||
          petData.userId ||
          petData.propietario?.id;
        if (ownerId) {
          try {
  ...
  const getSpeciesIcon = () => {
    ...
    if (s.includes("perro") || s.includes("dog"))
      return <Dog size={40} className="text-[#0A2540]" />;
    if (s.includes("gato") || s.includes("cat"))
      return <Cat size={40} className="text-[#0A2540]" />;
    if (s.includes("ave") || s.includes("bird"))
      return <Bird size={40} className="text-[#0A2540]" />;
  ...
  if (pet.fechaNacimiento || pet.fecha_nacimiento || pet.birthDate) {
    const dob = new Date(
      pet.fechaNacimiento || pet.fecha_nacimiento || pet.birthDate,
    );
  ...
  vaccines.forEach((v) => {
    const nextDate = new Date(
      v.nextBoosterDate || v.fechaProximoRefuerzo || v.proximaFecha,
    );
    if (nextDate < today) {
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden relative group">
            ...
            <div className="px-6 pb-6 relative">
              ...
              <h1 className="text-2xl font-black text-[#0A2540] mb-1">
                {pet.nombre || pet.name}
              </h1>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    {owner.cedula || owner.document
                      ? `C.I: ${owner.cedula || owner.document}`
                      : ""}
                  </p>
                </div>
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-3 text-sm">
                    ...
                    <span className="font-medium text-slate-700 break-all">
                      {owner.email || owner.correo || "Email no registrado"}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Vaccination Status */}
          <div
            className={`rounded-2xl p-6 border shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4 transition-colors ${isVaccinationUpToDate ? "bg-emerald-50 border-emerald-100" : "bg-amber-50 border-amber-100"}`}
          >
            <div className="flex items-center gap-4">
              <div
                className={`p-3 rounded-2xl ${isVaccinationUpToDate ? "bg-emerald-100 text-emerald-600" : "bg-amber-100 text-amber-600"}`}
              >
              ...
              <div>
                ...
                <p
                  className={`text-sm font-medium ${isVaccinationUpToDate ? "text-emerald-700" : "text-amber-700"}`}
                >
                  {isVaccinationUpToDate
            ...
            {sortedHistory.length > 0 ? (
              <div className="space-y-4">
                {sortedHistory.map((consultation, index) => (
                  <div
                    ...
                    className="group p-4 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-white hover:border-blue-100 hover:shadow-sm transition-all"
                  >
              ...
              <div className="flex flex-col items-center justify-center p-8 bg-slate-50 rounded-xl border border-dashed border-slate-200 text-center">
                ...
                <p className="text-sm text-slate-400 mt-1">
                  Este paciente no tiene historial clínico registrado.
                </p>
              </div>
            )}
          </div>
