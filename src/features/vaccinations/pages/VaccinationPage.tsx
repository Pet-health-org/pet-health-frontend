...
export function VaccinationPage() {
  ...
  const handleAddRecord = (data: any) => {
    ...
    notify(
      "success",
      "Vacuna Registrada",
      "La aplicación ha sido registrada correctamente.",
    );
    setIsFormOpen(false);
  };
  ...
  const handleNotifyOwner = (petName: string, ownerName: string) => {
    notify(
      ...
      `Se ha enviado el recordatorio a ${ownerName} para la mascota ${petName}.`,
    );
  };
  ...
  const filteredUpcoming = upcoming.filter((r) => {
    ...
    const dateMatch = panelDateFilter
      ? new Date(r.nextBoosterDate).toISOString().startsWith(panelDateFilter)
      : true;
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500">
            Control de esquemas y recordatorios de vacunas.
          </p>
        </div>
      </div>
                  ...
                  {filteredUpcoming.length > 0 ? (
                    filteredUpcoming.map((r) => {
                      ...
                      return (
                        <tr
                          key={r.id}
                          className={`text-sm hover:bg-slate-50 transition-colors ${isExpired ? "bg-red-50/50" : "bg-amber-50/30"}`}
                        >
                          <td className="p-4 font-bold text-[#0A2540]">
              ...
              <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {filteredPets.map((p) => (
                  ...
                  >
                    ...
                    <ChevronRight
                      size={16}
                      className={`text-slate-300 group-hover:text-blue-400 transition-colors ${selectedPet?.id === p.id ? "text-blue-500" : ""}`}
                    />
                  </button>
                ))}
              </div>
          ...
          <div className="lg:col-span-2">
            {selectedPet ? (
              <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-6">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {records.filter((r) => r.petId === selectedPet.id)
                        .length > 0 ? (
                        records
                          ...
                          .map((r) => (
                            <tr key={r.id} className="text-sm">
                              ...
                              <td className="p-3">
                                <span
                                  className={`px-2 py-1 rounded-full font-bold text-[10px] ${new Date(r.nextBoosterDate) < new Date() ? "bg-red-100 text-red-700" : "bg-blue-100 text-blue-700"}`}
                                >
                                  {new Date(
                                    r.nextBoosterDate,
                                  ).toLocaleDateString()}
              ...
              <div className="h-full min-h-[400px] flex flex-col items-center justify-center bg-slate-50 border border-dashed border-slate-300 rounded-xl text-slate-400">
                ...
                <p className="text-sm">
                  Para ver y gestionar su historial de vacunación.
                </p>
              </div>
            )}
          </div>
