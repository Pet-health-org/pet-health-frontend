  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        const speciesData = Object.entries(speciesMap).map(
          ([label, count], i) => ({
            label,
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500">
            Análisis dinámico basado en la base de datos actual.
          </p>
        </div>
      </div>
        ...
        <div className="bg-white p-12 rounded-2xl border-2 border-dashed border-slate-200 text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-slate-500 max-w-md mx-auto">
              Se requieren pacientes registrados e historias clínicas para
              generar métricas estadísticas precisas.
            </p>
          </div>
        </div>
      ) : (
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
            ...
            <div className="space-y-4 pt-2">
              {stats.commonDiseases.length > 0 ? (
                stats.commonDiseases.map((d, i) => (
                  <ReportBar
                    key={i}
                    label={d.label}
                    value={d.value}
                    total={stats.totalPatients || 1}
                    color={d.color}
                  />
                ))
              ) : (
...
function LegendItem({ label, value, color }: any) {
  return (
    <div className="flex items-center gap-2 p-2 bg-slate-50 rounded-lg border border-slate-100">
      ...
      <span className="text-[11px] text-slate-500 font-bold uppercase truncate">
        {label}
      </span>
...
function CoverageCard({ label, percentage }: any) {
  return (
    <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 text-center space-y-2 group hover:border-emerald-200 transition-all">
      ...
      <div className="text-3xl font-black text-[#0A2540] group-hover:scale-110 transition-transform">
        {percentage}%
      </div>
      <div className="w-full h-1.5 bg-white rounded-full overflow-hidden border border-slate-200">
        <div
          className={`h-full ${percentage > 80 ? "bg-emerald-500" : "bg-amber-400"} transition-all duration-1000`}
          style={{ width: `${percentage}%` }}
        ></div>
      </div>
    </div>
