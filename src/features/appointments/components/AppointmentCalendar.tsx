...
import { Clock, User, ChevronLeft, ChevronRight, Calendar as CalendarIcon, Filter } from 'lucide-react';
...
export function AppointmentCalendar({ 
  ...
  const blockedSlots = useMemo(() => [
    ...
    { day: 3, time: '15:00', reason: 'Mantenimiento' },
  ], []);
  ...
  const getStatus = (date: string, time: string) => {
    ...
    const blocked = blockedSlots.find(b => b.day === dayOfWeek && b.time === time);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex flex-col lg:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => navigateDate(view === 'day' ? -1 : -7)} className="p-2 hover:bg-white border border-transparent hover:border-slate-200 rounded-full text-slate-600 transition-all">
              <ChevronLeft size={20} />
            </button>
            <button onClick={goToToday} className="px-3 py-1.5 text-sm font-semibold text-[#0A2540] hover:bg-white rounded-md transition-all border border-transparent hover:border-slate-200">
              Hoy
            </button>
            <button onClick={() => navigateDate(view === 'day' ? 1 : 7)} className="p-2 hover:bg-white border border-transparent hover:border-slate-200 rounded-full text-slate-600 transition-all">
          ...
          <h3 className="text-lg font-bold text-slate-800 ml-2 capitalize">
            {new Intl.DateTimeFormat('es-ES', { month: 'long', year: 'numeric' }).format(new Date(selectedDate + 'T00:00:00'))}
          </h3>
        </div>
          ...
          <div className="relative flex-1 lg:flex-none">
            ...
            >
              ...
              {veterinarians.map((v: any) => (
                <option key={v.id} value={v.id}>Dr/a. {v.username || v.firstName}</option>
              ))}
            </select>
          </div>
          <input 
      ...
      <div className="overflow-auto max-h-[600px]">
        <div className={`grid ${view === 'week' ? 'grid-cols-[80px_repeat(6,1fr)]' : 'grid-cols-[80px_1fr]'} min-w-[800px] lg:min-w-0`}>
          {/* Header Row */}
          ...
          {view === 'week' ? weekDays.map(date => (
            <div key={date} className={`h-12 border-b border-r border-slate-100 bg-slate-50/80 flex flex-col items-center justify-center sticky top-0 z-10 backdrop-blur-sm ${date === new Date().toISOString().split('T')[0] ? 'bg-blue-50/50' : ''}`}>
              <span className="text-[10px] font-bold text-slate-400 uppercase">{formatDayName(date).split(' ')[0]}</span>
              <span className={`text-sm font-bold ${date === new Date().toISOString().split('T')[0] ? 'text-blue-600' : 'text-slate-700'}`}>{formatDayName(date).split(' ')[1]}</span>
            </div>
          )) : (
            <div className="h-12 border-b border-slate-100 bg-slate-50/80 flex flex-col items-center justify-center sticky top-0 z-10 backdrop-blur-sm">
              <span className="text-[10px] font-bold text-slate-400 uppercase">{formatDayName(selectedDate).split(' ')[0]}</span>
              <span className="text-lg font-black text-[#0A2540] -mt-1">{formatDayName(selectedDate).split(' ')[1]}</span>
            </div>
          )}
              ...
              {view === 'week' ? weekDays.map(date => {
                ...
                return (
                  <CalendarCell 
                    key={`${date}-${time}`}
                    status={status}
                    onClick={() => status.type === 'available' && onAddClick({ date, time, vetId: selectedVetId !== 'all' ? selectedVetId : '' })}
                  />
                );
              }) : (
                <CalendarCell 
                  ...
                  isDayView
                  onClick={() => getStatus(selectedDate, time).type === 'available' && onAddClick({ date: selectedDate, time, vetId: selectedVetId !== 'all' ? selectedVetId : '' })}
                />
              )}
            </React.Fragment>
          ))}
...
function CalendarCell({ status, onClick, isDayView }: { status: any, onClick: () => void, isDayView?: boolean }) {
        ...
        <div className="h-full w-full bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg p-2 text-white shadow-lg shadow-blue-500/20 flex flex-col justify-between group transition-all hover:scale-[1.02] cursor-default overflow-hidden">
          <div>
            <div className="flex justify-between items-start">
              <span className="text-[10px] font-bold opacity-80 uppercase tracking-tighter">Cita Médica</span>
              <div className="w-1.5 h-1.5 bg-white rounded-full animate-pulse"></div>
            </div>
            <p className="text-xs font-bold mt-1 line-clamp-1">{status.data.reason}</p>
          </div>
          <div className="flex items-center gap-1 mt-1 border-t border-white/20 pt-1">
            <User size={10} className="opacity-70" />
            <span className="text-[9px] font-medium opacity-90 truncate">Ref: {status.data.petId.split('-')[0]}</span>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="h-24 border-b border-r border-slate-100 p-1 bg-slate-50/30">
        <div className="h-full w-full bg-gradient-to-br from-amber-400 to-amber-500 rounded-lg p-2 text-white shadow-lg shadow-amber-500/20 flex flex-col justify-center items-center text-center opacity-90">
          ...
          <span className="text-[10px] font-bold uppercase">{status.data.reason}</span>
