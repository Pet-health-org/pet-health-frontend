...
export function useConsultations() {
  ...
  const validateVitals = useCallback((species: Species, vitals: VitalSigns) => {
    const ranges =
      SPECIES_VITAL_RANGES[species] || SPECIES_VITAL_RANGES["Otros"];
    const results = {
      temperature:
        vitals.temperature < ranges.temperature.min ||
        vitals.temperature > ranges.temperature.max,
      heartRate:
        vitals.heartRate < ranges.heartRate.min ||
        vitals.heartRate > ranges.heartRate.max,
      respiratoryRate:
        vitals.respiratoryRate < ranges.respiratoryRate.min ||
        vitals.respiratoryRate > ranges.respiratoryRate.max,
    };
    return {
      isValid:
        !results.temperature && !results.heartRate && !results.respiratoryRate,
      results,
    };
  }, []);
  ...
  const addConsultation = useCallback(
    async (data: any) => {
      ...
      try {
        ...
        const response = await createHistoriaClinica(payload);
