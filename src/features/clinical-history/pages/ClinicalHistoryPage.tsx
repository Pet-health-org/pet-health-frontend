...
export function ClinicalHistoryPage() {
  ...
  const { records: vaccineRecords, refresh: refreshVaccines } = useVaccines();
  ...
  const handleSubmitConsultation = async (
    ...
    try {
      const createdConsultation = await addConsultation(data);
      notify("success", "Éxito", "Consulta Registrada");
      return createdConsultation;
    } catch (error) {
      console.error("Error adding consultation:", error);
      notify("error", "Error", "Failed to add consultation.");
    }
  };

  const petVaccines = selectedPet
    ? vaccineRecords.filter((v) => v.petId === selectedPet.id)
    : [];
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500">
            Consulta y registro de atenciones veterinarias.
          </p>
        </div>
      </div>
      ...
      {!selectedPet ? (
        <div className="max-w-2xl mx-auto space-y-6 pt-12">
          <div className="text-center space-y-2">
            ...
            <p className="text-slate-500">
              Ingrese el nombre o ID de la mascota para ver su historia.
            </p>
          </div>
