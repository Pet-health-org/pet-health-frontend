...
interface ConsultationFormProps {
  pet: Pet;
  onClose: () => void;
  onSubmit: (data: Omit<Consultation, 'id' | 'registrationDate'> & { vitalsJustification?: string }) => Promise<void> | void;
...
export function ConsultationForm({ pet, onClose, onSubmit, isSubmitting }: ConsultationFormProps) {
  ...
  const { items: inventoryItems, recordMovement } = useInventory();
  ...
  const [vitalsStatus, setVitalsStatus] = useState<Record<string, 'normal' | 'warning' | 'critical'>>({});
  const ranges = SPECIES_VITAL_RANGES[pet.species as keyof typeof SPECIES_VITAL_RANGES] || SPECIES_VITAL_RANGES['Otros'];
  const hasOutlierVitals = Object.values(vitalsStatus).some(status => status !== 'normal');
  ...
  useEffect(() => {
    ...
    setVitalsStatus({
      temperature: evaluateVital(formData.vitals.temperature, ranges.temperature.min, ranges.temperature.max),
      heartRate: evaluateVital(formData.vitals.heartRate, ranges.heartRate.min, ranges.heartRate.max),
      respiratoryRate: evaluateVital(formData.vitals.respiratoryRate, ranges.respiratoryRate.min, ranges.respiratoryRate.max),
    });
  }, [formData.vitals, ranges]);

  useEffect(() => {
    if (vaccineData.vaccineName && scheme) {
      const vaccineInfo = scheme.vaccines.find(v => v.name === vaccineData.vaccineName);
      if (vaccineInfo) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (hasOutlierVitals && !formData.vitalsJustification.trim()) {
      alert("Por favor, justifique los valores anormales de las constantes vitales.");
      return;
    }
    ...
    try {
      ...
      if (applyVaccine && vaccineData.vaccineName) {
        ...
        // Find inventory and discount
        const invItem = inventoryItems.find(i => i.category === 'Vacuna' && i.name.toLowerCase().includes(vaccineData.vaccineName.toLowerCase()));
        if (invItem) {
          await recordMovement(invItem.id, 'Salida', 1, `Aplicación en consulta - ${pet.name}`);
        }
      }
      onClose();
    } catch (error) {
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto custom-scrollbar space-y-6">
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    />
                    <div className={`text-[10px] ${vitalsStatus.temperature !== 'normal' ? 'font-bold' : 'text-slate-400'}`}>
                      Rango normal: {ranges.temperature.min} - {ranges.temperature.max}
                    </div>
                  </div>
                  <div className="space-y-1">
                    ...
                    />
                    <div className={`text-[10px] ${vitalsStatus.heartRate !== 'normal' ? 'font-bold' : 'text-slate-400'}`}>
                      Rango normal: {ranges.heartRate.min} - {ranges.heartRate.max}
                    </div>
                  </div>
                  <div className="space-y-1">
                    ...
                    />
                    <div className={`text-[10px] ${vitalsStatus.respiratoryRate !== 'normal' ? 'font-bold' : 'text-slate-400'}`}>
                      Rango normal: {ranges.respiratoryRate.min} - {ranges.respiratoryRate.max}
                    </div>
                  </div>
                  <div className="space-y-1">
                ...
                {hasOutlierVitals && (
                  <div className="mt-4 space-y-1 animate-in slide-in-from-top-2">
                    <label className="text-sm font-bold text-amber-700 flex items-center gap-1">
                      <AlertTriangle size={14} /> Justificación Constantes Anormales *
                    </label>
                    <textarea
                      ...
                      onChange={(e) => setFormData({ ...formData, vitalsJustification: e.target.value })}
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 transition-all">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <span className="font-bold text-[#0A2540] flex items-center gap-2">
                <Syringe size={18} /> ¿Registrar aplicación de vacuna en esta consulta?
              </span>
            </label>
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 animate-in slide-in-from-top-2">
                ...
                <div className="space-y-1 relative">
                  ...
                  <input
                    ...
                    onChange={(e) => setVaccineData({ ...vaccineData, expiryDate: e.target.value })}
                    required={applyVaccine}
                  />
                  {vaccineWarning && (
                    <div className="flex items-center gap-1 text-[10px] text-amber-700 mt-1 font-bold">
                      <AlertCircle size={12} /> El lote vence en menos de 30 días
                    </div>
                  )}
                </div>
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => setVaccineData({ ...vaccineData, nextBoosterDate: e.target.value })}
                    required={applyVaccine}
                  />
                </div>
              </div>
          ...
          <div className="pt-4 border-t border-slate-100 flex gap-3 justify-end">
            ...
            <button type="submit" disabled={isSubmitting || localSubmitting} className="px-6 py-2 bg-[#0A2540] text-white rounded-lg font-medium hover:bg-[#113255] transition-colors shadow-lg shadow-[#0A2540]/20 flex items-center gap-2">
              {(isSubmitting || localSubmitting) ? 'Guardando...' : 'Finalizar Consulta'}
            </button>
          </div>
        </form>
      </div>
