...
describe('LowStockAlerts Component', () => {
  ...
  it('no debe renderizar nada si no hay alertas', async () => {
    ...
    vi.mocked(findBajoStock).mockResolvedValueOnce({ data: [] });
  ...
  it('debe renderizar la lista de productos críticos', async () => {
    const mockData = [
      { id: '1', nombreProducto: 'Vacuna A', stockActual: 0, stockMinimo: 10, proveedor: { nombre: 'Prov 1' } },
      { id: '2', nombreProducto: 'Jeringas', stockActual: 5, stockMinimo: 20, proveedor: { nombre: 'Prov 2' } },
    ];
    
    vi.mocked(findBajoStock).mockResolvedValueOnce({ data: mockData });
  ...
  it('debe llamar a notify al hacer clic en Generar Orden', async () => {
    const mockData = [
      { id: '1', nombreProducto: 'Ibuprofeno', stockActual: 2, stockMinimo: 10 }
    ];
    ...
    expect(mockNotify).toHaveBeenCalledWith('success', 'Orden Generada', 'Se ha generado una orden de compra para: Ibuprofeno');
  });
});
