...
describe('Home Component', () => {
  ...
  it('debe renderizar el landing page sin el modal abierto', () => {
    ...
    expect(screen.getByText(/Gestión veterinaria moderna y eficiente/i)).toBeInTheDocument();
  ...
  it('debe abrir el modal de login al hacer clic en Entrar al Sistema', () => {
    ...
    const btnEntrar = screen.getByRole('button', { name: /Entrar al Sistema/i });
  ...
  it('debe iniciar sesión exitosamente, notificar y redirigir', async () => {
    ...
    vi.mocked(authLogin).mockResolvedValueOnce({ data: { access_token: 'fake-token' } });
    
    // Encontrar y enviar el form
    const btnSubmit = screen.getAllByRole('button', { name: /Entrar al Sistema/i })[1];
    fireEvent.click(btnSubmit);
    
    await waitFor(() => {
      // Verificar que authLogin fue llamado con admin/Admin123! (valores por defecto del state)
      expect(authLogin).toHaveBeenCalledWith({ username: 'admin', password: 'Admin123!' });
      
      // Verificar que useAuth.login fue llamado
      expect(mockLogin).toHaveBeenCalledWith('fake-token');
      
      // Verificar notificación de éxito
      expect(mockNotify).toHaveBeenCalledWith('success', 'Bienvenido', 'Has iniciado sesión correctamente.');
  ...
  it('debe bloquearse después de 3 intentos fallidos', async () => {
    ...
    // Intento 1
    fireEvent.click(btnSubmit);
    await waitFor(() => expect(mockNotify).toHaveBeenCalledWith('error', 'Error de acceso', 'Credenciales incorrectas. Intento 1/3'));
    
    // Intento 2
    fireEvent.click(btnSubmit);
    await waitFor(() => expect(mockNotify).toHaveBeenCalledWith('error', 'Error de acceso', 'Credenciales incorrectas. Intento 2/3'));
    
    // Intento 3
    fireEvent.click(btnSubmit);
    await waitFor(() => expect(mockNotify).toHaveBeenCalledWith('error', 'Acceso Bloqueado', 'Por seguridad, el acceso ha sido bloqueado tras 3 intentos fallidos.'));
    
    // Verificar que el formulario se bloqueó
    expect(screen.getByText('Acceso bloqueado por seguridad tras 3 intentos fallidos.')).toBeInTheDocument();
    
    const usernameInput = screen.getByPlaceholderText('Nombre de usuario');
    expect(usernameInput).toBeDisabled();
    
    const disabledBtn = screen.getByRole('button', { name: /Cuenta Bloqueada/i });
