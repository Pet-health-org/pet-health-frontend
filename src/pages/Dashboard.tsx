      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500">
            Bienvenido/a, {user?.username}. Aquí está el resumen de hoy.
          </p>
        </div>
          ...
          <div className="space-y-4">
            {recentAppointments.length > 0 ? (
              recentAppointments.map((cita) => (
                <div
                  key={cita.id}
                  className="flex items-center p-3 hover:bg-slate-50 rounded-lg transition-colors border border-slate-100"
                >
...
function SummaryCard({
  ...
  return (
    <div
      className={`bg-white p-6 rounded-xl shadow-sm border ${alert ? "border-red-200 shadow-red-100" : "border-slate-200"} transition-all hover:shadow-md`}
    >
      <div className="flex justify-between items-start mb-4">
        <div
          className={`p-3 rounded-lg ${alert ? "bg-red-50" : "bg-slate-50"}`}
        >
          {icon}
        </div>
        {trend && (
          <span
            className={`text-xs font-semibold px-2 py-1 rounded-full ${trendUp ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}
          >
            {trend}
          </span>
        )}
